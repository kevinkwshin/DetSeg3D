...
def generate_train_transform(patch_size, amp=True, debug=False, min_size=10, merge_distance_mm=20.0, intensity_min=0, intensity_max=120):
    """
    ...
    Note: batch_size is controlled by DataLoader, not by this transform.
    """
    ...
    train_transforms = Compose([
        ...
        RandCropByPosNegLabeld(
            ...
            num_samples=2,  # Number of PAIRS to generate (pos=1, neg=1 → generates 2 patches total)
...
def train_epoch(detector, loader, optimizer, device, epoch, scaler=None, amp=True, verbose=True):
    ...
    for batch_idx, batch_data in enumerate(pbar):
        ...
        # RandCropByPosNegLabeld with pos=1, neg=1 returns nested list
        # Each input image generates 2 patches (1 positive + 1 negative)
        # Flatten: [[patch1_pos, patch1_neg], [patch2_pos, patch2_neg]] -> [patch1_pos, patch1_neg, patch2_pos, patch2_neg]
        if len(batch_data) > 0 and isinstance(batch_data[0], list):
...
def validate(detector, loader, device, coco_metric, amp=True, verbose=True, save_debug=False, output_dir=None, epoch=0):
    ...
    for img_idx in range(len(results_metric)):
        ...
        # Handle different result formats from matching_batch
        if isinstance(result, dict):
            # MONAI's matching_batch returns dict with IoU threshold as key
            # Format: {np.int64(0): array([...]), np.int64(1): array([...]), ...}
            # where key is IoU threshold index and value is match array
            
            # Try to get matches for first IoU threshold (0.1)
            matches = None
            if iou_idx in result:
                matches = result[iou_idx]
            elif np.int64(iou_idx) in result:
                matches = result[np.int64(iou_idx)]
            elif len(result) > 0:
                # Get first key (should be IoU threshold 0)
                first_key = list(result.keys())[iou_idx] if iou_idx < len(result) else list(result.keys())[0]
                matches = result[first_key]
            
            if matches is not None:
                # CRITICAL FIX: matches might still be a dict, need to unwrap
                # If matches is still a dict, try to extract the array from it
                if isinstance(matches, dict):
                    # Try common keys
                    for possible_key in ['dtMatches', 'matches', 0, np.int64(0)]:
                        if possible_key in matches:
                            matches = matches[possible_key]
                            break
                    
                    # If still a dict, skip this image
                    if isinstance(matches, dict):
                        if verbose and img_idx == 0:
                            print(f"⚠️  Warning: Could not extract match array from nested dict for image {img_idx}")
                            print(f"   Keys available: {list(matches.keys())}")
                        continue
                
                # Now matches should be an array-like object
                if hasattr(matches, '__len__') and not isinstance(matches, dict):
                    try:
                        # matches is array of GT indices matched to each detection
                        # 0 = unmatched (FP), >0 = matched to GT (TP)
                        matches_array = np.array(matches)
                        tp_count += int((matches_array > 0).sum())  # Matched detections (TP)
                        fp_count += int((matches_array == 0).sum())  # Unmatched detections (FP)
                    except (ValueError, TypeError) as e:
                        if verbose and img_idx == 0:
                            print(f"⚠️  Warning: Could not convert matches to array for image {img_idx}: {e}")
                        continue
                
        elif hasattr(result, '__len__') and len(result) > 0:
            # Array format: [num_iou_thresholds, num_predictions]
            if len(result) > iou_idx:
                matched = result[iou_idx]
                if hasattr(matched, 'sum'):
                    try:
                        tp_count += int((matched > 0).sum())
                        fp_count += int((matched == 0).sum())
                    except (ValueError, TypeError) as e:
                        if verbose and img_idx == 0:
                            print(f"⚠️  Warning: Could not process matches for image {img_idx}: {e}")
                        continue
    ...
    # Debug matching_batch results (always show for troubleshooting)
    if verbose:
        print(f"\n🔍 Detection Counting Debug:")
        print(f"   - Total predictions: {total_pred}")
        print(f"   - Total GT boxes: {total_gt}")
        print(f"   - TP counted: {tp_count}")
        print(f"   - FP counted: {fp_count}")
        print(f"   - FN calculated: {fn_count}")
        
        if save_debug:
            print(f"\n🔍 Matching batch results structure:")
            ...
            if len(results_metric) > 0:
                ...
                if isinstance(results_metric[0], dict):
                    ...
                    # Show first match array for debugging
                    first_key = list(results_metric[0].keys())[0]
                    first_matches = results_metric[0][first_key]
                    print(f"   - First match value type: {type(first_matches)}")
                    if hasattr(first_matches, '__len__') and not isinstance(first_matches, dict):
                        try:
                            first_matches_array = np.array(first_matches)
                            print(f"   - First match array (key={first_key}): shape={first_matches_array.shape}, dtype={first_matches_array.dtype}")
                            if len(first_matches_array) > 0:
                                print(f"   - Sample values: {first_matches_array.flat[:10]}")
                        except Exception as e:
                            print(f"   - Could not convert to array: {e}")
                    else:
                        print(f"   - First match value: {first_matches}")
...
To modify settings, edit config.yaml directly.
    ...
    if is_main_process:
        ...
        print(f"   - Images per GPU per iteration: {args.batch_size}")
        print(f"   - Patches per image: 2 (1 pos + 1 neg)")
        print(f"   - Total patches per iteration: {args.batch_size * world_size * 2}")
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.val_interval == 0:
            ...
            if is_main_process:
                ...
                # ========================================================================
                # TEMPORARY: Save checkpoint every validation (without metric checking)
                # This is a workaround while validation metrics are being debugged.
                # TODO: Re-enable metric-based saving after fixing detection statistics
                # ========================================================================
                print(f"\n💾 Saving checkpoint (Epoch {epoch+1})...")
                print(f"   ⚠️  TEMPORARY: Saving without metric check (for debugging)")
                
                # Save epoch-specific checkpoint
                save_path_epoch = os.path.join(args.output_dir, f"checkpoint_epoch{epoch+1:04d}_{args.backbone}.pth")
                # Also save as latest checkpoint (overwrite)
                save_path_latest = os.path.join(args.output_dir, f"latest_model_{args.backbone}.pth")
                ...
                print(f"   ✅ Saved: {os.path.basename(save_path_epoch)}")
                
                # Save latest checkpoint (overwrite)
                torch.save(checkpoint_data, save_path_latest)
                print(f"   ✅ Saved: {os.path.basename(save_path_latest)} (latest)")
                
                # Keep track of best F1 for display purposes
                if f1_score > best_f1:
                    best_f1 = f1_score
                    best_epoch = epoch
                    print(f"   🏆 New best F1: {best_f1:.1%} at epoch {best_epoch+1}")
                
                # ========================================================================
                # COMMENTED OUT: Original metric-based saving
                # Uncomment this after fixing validation metrics
                # ========================================================================
                # if f1_score > best_f1:
                #     best_f1 = f1_score
                #     best_epoch = epoch
    ...
    if is_main_process:
        ...
        print(f"\n⚠️  Note: Checkpoints saved without metric check (temporary for debugging)")
        print(f"   - Latest checkpoint: latest_model_{args.backbone}.pth")
        print(f"   - Epoch checkpoints: checkpoint_epoch####_{args.backbone}.pth")
        print(f"\n📊 Best F1 score observed: {best_f1:.1%} at epoch {best_epoch+1}")
        print(f"   (This is for reference only - all checkpoints were saved)")
        print(f"{'='*60}\n")
        if writer is not None:
            writer.close()
